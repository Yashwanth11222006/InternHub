...
export default function ApplicantsPage() {
    ...
    const [internship, setInternship] = useState<{ title: string } | null>(null);
    const [applicantsData, setApplicantsData] = useState<FlattenedApplicant[]>([]);
    ...
    useEffect(() => {
        const fetchApplicants = async () => {
            setLoading(true);
            try {
                const data = await api.recruiter.getApplicants(id);

                setInternship({ title: data.internship_title });

                // Flatten data for easier use
                const flattened: FlattenedApplicant[] = data.applications.map((app: any) => ({
                    id: app.id,
                    name: app.student?.full_name || 'Unknown',
                    email: app.student?.email,
                    university: app.student?.university || 'Not Provided',
    ...
    const handleStatusUpdate = async (appId: string, newStatus: string) => {
        ...
        try {
            await api.applications.updateStatus(appId, newStatus.toLowerCase() as ApplicationStatus);
    ...
    return (
        <div className="space-y-10 animate-fade-in pb-20 pt-10 px-4 md:px-0">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
                <div>
                    <Button variant="ghost" className="mb-4 text-black/60 hover:text-black p-0 h-auto font-bold" onClick={() => router.back()}>
                    ...
                    <p className="text-black/60 mt-2 font-bold italic flex items-center gap-1.5 uppercase tracking-widest text-[10px]">
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            statusFilter === status
                                ...
                                : 'bg-white text-black/60 border-slate-200 hover:border-black hover:text-black'
                        )}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <Card key={a.id} className="p-8 border-none shadow-sm hover:shadow-2xl transition-all rounded-[40px] bg-white flex flex-col group cursor-pointer" onClick={() => setSelectedApplicantId(a.id)}>
                        <div className="flex items-start justify-between mb-8">
                            <div className="flex gap-4">
                                <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-black/40 font-black text-xl group-hover:bg-primary/5 group-hover:text-primary transition-colors">
                                    {a.name[0]}
                                </div>
                                <div className="pt-1">
                                    <p className="font-black text-black leading-none mb-1.5">{a.name}</p>
                                    <p className="text-[10px] font-bold text-black/60 uppercase tracking-widest">{a.university}</p>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-8 flex-1">
                            {a.skills.slice(0, 3).map((s: string) => (
                                <Badge key={s} variant="secondary" className="px-3 py-1 bg-slate-50 text-black/60 border-none text-[10px] font-bold rounded-lg uppercase tracking-tight">{s}</Badge>
                            ))}
                            {a.skills.length > 3 && <span className="text-[9px] font-black text-black/40 uppercase self-center ml-1">+{a.skills.length - 3}</span>}
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                            ...
                            <div className="text-[10px] font-black text-black uppercase tracking-widest group-hover:gap-2 flex items-center gap-1 transition-all">
                ...
                {filtered.length === 0 && (
                    <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-4 border-dashed border-slate-50">
                        <User className="w-12 h-12 text-black/10 mx-auto mb-4" />
                        <h3 className="text-xl font-black text-black/40">No candidates found</h3>
                    </div>
                )}
            </div>
            ...
            <Dialog open={!!selectedApplicantId} onOpenChange={(open) => !open && setSelectedApplicantId(null)}>
                <DialogContent className="max-w-2xl rounded-[40px] border-none shadow-2xl p-10 bg-white">
                    <DialogHeader>
                        <DialogTitle className="text-3xl font-black flex items-center gap-4 text-black mb-2">
                            <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary">
                                <User className="w-6 h-6" />
                            </div>
                            Candidate Profile
                        </DialogTitle>
                        <DialogDescription className="text-black/60 font-bold italic">
                        ...
                        <div className="space-y-8 pt-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-6">
                                    <div>
                                        <p className="text-[10px] font-black text-black/60 uppercase tracking-widest mb-3">Contact Information</p>
                                        <div className="space-y-3">
                                            <p className="text-xl font-black text-black">{applicant.name}</p>
                                            <p className="text-sm font-bold text-black/70 flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-black/60 uppercase tracking-widest mb-3">Expertise</p>
                                        <div className="flex flex-wrap gap-2">
                                            {applicant.skills.map((s: string) => (
                                                <Badge key={s} variant="secondary" className="px-3 py-1 bg-slate-100 text-black/70 border-none font-bold uppercase text-[9px] tracking-tight">{s}</Badge>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div>
                                        <p className="text-[10px] font-black text-black/60 uppercase tracking-widest mb-3">Resume & Links</p>
                                        <div className="space-y-3">
                                            {applicant.resumeUrl ? (
                                                ...
                                                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100 text-black/50 text-sm font-bold italic">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-black/60 uppercase tracking-widest mb-3">Cover Letter / Pitch</p>
