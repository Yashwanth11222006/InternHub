...
export default function ManageInternshipsPage() {
    ...
    const toggleStatus = async (id: string, currentStatus: string) => {
        ...
        try {
            ...
            setInternships(prev => prev.map(i => i.id === id ? { ...i, status: nextStatus } : i));
        } catch (error: any) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <p className="font-black text-slate-400 uppercase tracking-widest text-xs">Loading Listings...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-10 animate-slide-up pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div>
                    <h1 className="text-4xl font-black text-[#383838] tracking-tight font-sans">Manage Listings</h1>
                    <p className="text-slate-500 mt-2 font-bold uppercase tracking-widest text-xs">Review, edit, or close your published opportunities.</p>
                </div>
                        ...
                        <Card key={internship.id} className="p-0 border border-slate-200/60 shadow-sm hover:shadow-xl transition-all duration-500 rounded-[40px] bg-white overflow-hidden group border-b-4 hover:border-b-primary">
                            <div className="flex flex-col lg:flex-row">
                                <div className="flex-1 p-8">
                                    ...
                                    <h3 className="text-3xl font-black text-[#383838] mb-4 group-hover:text-primary transition-colors tracking-tight font-sans leading-none">{internship.title}</h3>
                                    <div className="flex flex-wrap gap-8 mt-8">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Applicants</p>
                                                <p className="text-xl font-black text-[#383838] leading-none">{internship.applicantCount}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xl font-black text-[#383838] leading-none">{new Date(internship.deadline).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-50/50 lg:w-80 p-8 flex flex-col justify-center gap-4 border-t lg:border-t-0 lg:border-l border-slate-100">
                                    ...
                                    <Button
                                        variant="ghost"
                                        className="w-full h-12 rounded-xl font-black text-slate-400 hover:text-rose-600 hover:bg-rose-50"
